    ...
    return (
        <div className="tight-table-wrapper">
            ...
            <table className="tightTable">
                ...
                {GuestRows(guests.filter(guest => guest.main?.toString() && guest.dessert?.toString()), "chosen")}
                {GuestRows(guests.filter(guest => guest.main === null || guest.dessert === null), "pending")}
            </table>
            <div className="admin-course-summary">
                <h2>Mains</h2>
                {
                    mains.map(main => {
                        const total = guests.filter(guest => guest.main === main.value).length;
                ...
                <h2>Desserts</h2>
                {
                    desserts.map(dessert => {
                        const total = guests.filter(guest => guest.dessert === dessert.value).length;
